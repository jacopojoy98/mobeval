...
class EvalConfig:
    # data & splits
    split_by: str = "time"                      # 'time' (UniTE 8:1:1 chronological) or 'user'
...
class EvalContext:
    def __init__(self, dataset: MobilityDataset, cfg: EvalConfig):
        ...
        self.splits = dataset.split(cfg.split_by, cfg.split_ratios, cfg.split_seed)
        ...
        all_sp = detect_staypoints(dataset, cfg.staypoint_dist_m, cfg.staypoint_time_s)
    ...
    def summary(self) -> str:
        ...
        for s in ("train", "val", "test"):
            lines.append(f"  {s:5s}: points={len(self.splits[s].points):7d} "
